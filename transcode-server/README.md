# Tessel Transcode Server

A tiny self-hosted companion for the [Tessel TV app](../). It runs on a small
...
```bash
docker run -d --name tessel-transcode --restart unless-stopped \
  -p 8200:8200 -v "$PWD/tessel-data:/data" --device /dev/dri \
  ghcr.io/patrickst1991/tessel-transcode:latest
```
...
[Releases page](https://github.com/PatrickSt1991/tessel-tizen-tv/releases),
under the latest `transcode-v*` tag:

| OS | Asset | Bundled ffmpeg? |
|---|---|---|
| Windows x64 | `tessel-transcode-windows-amd64.zip` | ✅ included (BtbN GPL build) |
| Linux x64 | `tessel-transcode-linux-amd64.zip` | ✅ included (BtbN GPL build) |
| Linux ARM64 (Raspberry Pi 4/5, generic ARM) | `tessel-transcode-linux-arm64.zip` | ✅ included |
| macOS Apple Silicon | `tessel-transcode-darwin-arm64.zip` | ❌ run `brew install ffmpeg` |
| macOS Intel | `tessel-transcode-darwin-amd64.zip` | ❌ run `brew install ffmpeg` |

Unzip anywhere. On Windows + Linux that gets you `tessel-transcode(.exe)` plus
...
```bash
# Linux / macOS
DATA_DIR=./tessel-data ./tessel-transcode

# Windows (cmd.exe / PowerShell)
set DATA_DIR=%CD%\tessel-data
tessel-transcode.exe
```

**Upgrading from VLC TV Transcode Server?** The binary is now called
`tessel-transcode`, but keep `DATA_DIR` pointing at your existing folder (e.g.
`./vlc-data`) — that's where your share settings and pairing token live, so the
TV stays paired.
...
install — `./tessel-data` next to the binary is the easiest choice.
...
and `ffprobe(.exe)` in the same folder as `tessel-transcode(.exe)`.
...
| `WORK_DIR` | `/tmp/tessel-transcode` | scratch for HLS segments |
